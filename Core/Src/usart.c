...
uart_data uart_rx_data_t[UART_BUFFER_QUANTITY]; // ˫�������ݴ洢
volatile uint8_t uart_buff_ctrl = 0; // ����������
volatile uint32_t uart_rx_queue_full_count = 0;
/* USER CODE END 0 */
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
  if (huart->Instance == USART2) {
    ...
    if (UART_DMA_IDLE_RECEPT_QUEUEHandle != NULL) {
      ...
      if (xQueueSendFromISR(UART_DMA_IDLE_RECEPT_QUEUEHandle, &pUartData, &xHigherPriorityTaskWoken) != pdPASS) {
        uart_rx_queue_full_count++;
