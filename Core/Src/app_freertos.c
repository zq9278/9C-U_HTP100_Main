...
void MX_FREERTOS_Init(void) {
    ...
    ws2812_white_delayHandle = osTimerNew(ws2812_white_delay_callback, osTimerOnce, NULL,
    ...
    UART_DMA_IDLE_RECEPT_QUEUEHandle = osMessageQueueNew(3, sizeof(uart_data *),
                                                         &UART_DMA_IDLE_RECEPT_QUEUE_attributes);
...
void Uart_send_task(void *argument) {
    ...
    for (;;) {
        ...
        if (osMessageQueueGet(Battery_DATAHandle, &battery, NULL, 0) == osOK) {
            if ((lastBatteryValue == 0 || fabs(battery - lastBatteryValue) <= 10.0f) && (flag_200ms)) {
